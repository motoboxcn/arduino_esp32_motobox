...
3. **传感器数据 (sensors)**
   - **IMU数据 (imu)**: 三轴加速度(accel_x/y/z)、三轴陀螺仪(gyro_x/y/z)、姿态角(roll/pitch/yaw)
   - **罗盘数据 (compass)**: 航向(heading)、三轴磁场强度(mag_x/y/z)
...
3. **新增表**
   - **IMU数据表**：存储三轴加速度(accel_x/y/z)、三轴陀螺仪(gyro_x/y/z)、姿态角(roll/pitch/yaw)数据
   - **罗盘数据表**：存储航向(heading)、三轴磁场强度(mag_x/y/z)数据
   - **存储信息表**：存储SD卡大小(size_mb)和剩余空间(free_mb)
...
### IMU数据字段
- **加速度数据**：`accel_x`, `accel_y`, `accel_z` (单位：m/s²)
- **陀螺仪数据**：`gyro_x`, `gyro_y`, `gyro_z` (单位：rad/s)
- **姿态角数据**：`roll`, `pitch`, `yaw` (单位：度)
- **时间戳**：`timestamp` (毫秒)
- **有效性**：通过 `valid` 字段判断数据是否有效

### 罗盘数据字段
- **航向数据**：`heading` (单位：度，0-360)
- **磁场强度**：`mag_x`, `mag_y`, `mag_z` (单位：μT)
- **时间戳**：`timestamp` (毫秒)
- **有效性**：通过 `valid` 字段判断数据是否有效

### 数据可用性说明

**推荐设计：通过 `modules` 状态判断数据可用性**
...
```json
{
  "modules": {
    ...
    "ble": true,
    "gsm": true,
    "gnss": true,      // 如果为true，则location数据可用
    "imu": true,       // 如果为true，则sensors.imu数据可用
    "compass": true,   // 如果为true，则sensors.compass数据可用
    "sd": true,        // 如果为true，则storage数据可用
    "audio": false
  }
}
```
...
**数据可用性规则：**
- **位置数据 (location)**：`modules.gnss == true` 时可用
- **IMU数据 (sensors.imu)**：`modules.imu == true` 时可用
- **罗盘数据 (sensors.compass)**：`modules.compass == true` 时可用
- **存储信息 (storage)**：`modules.sd == true` 时可用
- **系统状态和模块状态**：始终存在，提供完整的系统信息

**订阅服务处理逻辑：**
```python
# 检查数据可用性
if payload.get("modules", {}).get("gnss", False):
    # 处理位置数据
    location = payload.get("location", {})
    
if payload.get("modules", {}).get("imu", False):
    # 处理IMU数据
    imu_data = payload.get("sensors", {}).get("imu", {})
    
if payload.get("modules", {}).get("compass", False):
    # 处理罗盘数据
    compass_data = payload.get("sensors", {}).get("compass", {})
```
